...
@section('content')
    @php
        ...
        if ($userRole == 'manager') {
            $config['back_url'] = '/manajer/project';
        } elseif ($userRole == 'karyawan') {
            $config['back_url'] = '/karyawan/project';
        } elseif ($userRole == 'admin-sdm') {
            $config['back_url'] = '/admin_sdm/project';
        }
                ...
                <div class="card custom-card border-top-card border-top-primary rounded-0 rounded-bottom">
                    <div class="card-header justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ $config['back_url'] }}" class="btn btn-light btn-sm btn-wave" data-bs-toggle="tooltip"
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                <td class="text-center text-muted">
                                                    {{ $startDate->translatedFormat('d F Y') }}</td>
                                                <td class="text-center fw-bold text-success">
                                                    {{ $startDate->translatedFormat('d F Y') }}</td>
                                                <td class="text-center"><span
                                                        class="badge bg-success-transparent rounded-pill"><i
                                                            class="ri-check-line"></i> OK</span></td>
                                            </tr>
                                            <tr>
                                                <td class="fw-semibold">Waktu Selesai</td>
                                                <td class="text-center text-muted">
                                                    {{ $deadlineDate->translatedFormat('d F Y') }}</td>
                                                <td class="text-center">
                                                    @if ($endDate)
                                                        <span
                                                            class="fw-bold text-dark">{{ $endDate->translatedFormat('d F Y') }}</span>
                                                    @else
                                                ...
                                                <td class="text-center">
                                                    @if ($endDate)
                                                        ...
                                                        @if ($diff > 0)
                                                            <span class="badge bg-danger-transparent rounded-pill">Telat
                                                                {{ $diff }} Hari</span>
                                                        @elseif($diff < 0)
                                                            <span class="badge bg-success-transparent rounded-pill">Cepat
                                                                {{ abs($diff) }} Hari</span>
                                                        @else
                                                            <span class="badge bg-info-transparent rounded-pill">Tepat
                                                                Waktu</span>
                                                        @endif
                                                    @else
                                                        @if ($endDate == '')
                                                            <span class="badge bg-danger-transparent rounded-pill">Belum
                                                                Terdefinisi</span>
                                                        @else
                                                            @if (\Carbon\Carbon::now()->gt($deadlineDate))
                                                                <span
                                                                    class="badge bg-danger-transparent rounded-pill">Overdue</span>
                                                            @else
                                                                <span class="badge bg-light text-dark border">On
                                                                    Track</span>
                                                            @endif
                                                        @endif
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center text-muted">
                                                    {{ round($startDate->diffInDays($deadlineDate)) }} Hari</td>
                                                        ...
                                                        @php
                                                            $percentageUsed =
                                                                ($startDate->diffInDays($endDate) /
                                                                    $startDate->diffInDays($deadlineDate)) *
                                                                100;
                                                            $efficiency = 100 - $percentageUsed;
                                                        @endphp
                                                        @if ($efficiency >= 0)
                                                            <span class="text-success fw-bold">+{{ round($efficiency) }}%
                                                                Efisien</span>
                                                        @else
                                                            <span class="text-danger fw-bold">{{ round($efficiency) }}%
                                                                Inefisien</span>
                                                        @endif
                                                    @else
                        ...
                        <form action="{{ $config['action_update_project'] }}" method="post">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="perusahaan_id" class="form-control" id="perusahaan_id" {{ $inputState }}>
                                    @foreach ($perusahaan as $row)
                                        <option value="{{ $row->id }}"
                                            {{ $project->perusahaan_id == $row->id ? 'selected' : '' }}>
                                            {{ $row->nama_perusahaan }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" name="status" id="status" {{ $inputState }}>
                                    <option value="belum" {{ $project->status == 'belum' ? 'selected' : '' }}>Belum
                                    </option>
                                    <option value="proses" {{ $project->status == 'proses' ? 'selected' : '' }}>Proses
                                    </option>
                                    <option value="selesai" {{ $project->status == 'selesai' ? 'selected' : '' }}>Selesai
                                    </option>
                                </select>
                            </div>
                                ...
                                <div class="col-6">
                                    ...
                                    <div class="input-group">
                                        <span class="input-group-text bg-light border-end-0"><i
                                                class="ri-calendar-line text-muted"></i></span>
                                        <input type="text" class="form-control border-start-0 ps-0" name="waktu_mulai"
                                            id="waktu_mulai" value="{{ $project->waktu_mulai }}" {{ $inputState }}>
                                    </div>
                                </div>
                                <div class="col-6">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control border-start-0 ps-0" name="deadline"
                                            id="deadline" value="{{ $project->deadline }}" {{ $inputState }}>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text bg-light border-end-0"><i
                                            class="ri-checkbox-circle-line text-success"></i></span>
                                    <input type="text" class="form-control border-start-0 ps-0" name="waktu_berakhir"
                                        id="waktu_berakhir" value="{{ $project->waktu_berakhir }}"
                                        placeholder="Belum Selesai" {{ $inputState }}>
                                </div>
                            </div>
                            ...
                            <div class="tab-pane" id="tasks" role="tabpanel">
                                ...
                                <div class="table-responsive">
                                    <table id="datatable-basic" class="table table-hover text-nowrap w-100">
                                        ...
                                        <tbody>
                                            @php
                                                $taskList = $config['is_manager']
                                                    ? $tasks
                                                    : $userstasks->map(fn($ut) => $ut->task);
                                            @endphp

                                            @forelse ($taskList as $item)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td><span class="fw-semibold text-dark">{{ $item->nama_task }}</span>
                                                    </td>
                                                    <td class="text-muted">
                                                        ...
                                                        <div class="d-flex justify-content-center gap-2">
                                                            @php
                                                                ...
                                                                if ($userRole == 'manager') {
                                                                    ...
                                                                    $detailRoute = route(
                                                                        'karyawan.detail.task',
                                                                        $item->id,
                                                                    );
                                                                } elseif ($userRole == 'admin-sdm') {
                                                                    $detailRoute = route(
                                                                        'admin_sdm.detail.task',
                                                                        $item->id,
                                                                    );
                                                                }
                                                            @endphp
                                                            <a href="{{ $detailRoute }}"
                                                                ...
                                                                data-bs-toggle="tooltip" title="Detail Task">
                                                            ...
                                                            @if ($config['is_manager'])
                                                                ...
                                                                <form
                                                                    action="{{ route('manajer.delete.task', $item->id) }}"
                                                                    method="POST" class="d-inline">
                                                                    @csrf @method('DELETE')
                                                                    <button type="button"
                                                                        class="btn btn-sm btn-icon btn-danger delete-task"
                                                                        data-id="{{ $item->id }}"
                                                                        data-nama_task="{{ $item->nama_task }}"
                                                                        title="Hapus">
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="5" class="text-center text-muted py-4">Belum ada task.
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                                    ...
                                                    <div class="d-flex align-items-center">
                                                        <div class="me-3">
                                                            <span class="avatar avatar-lg rounded-circle">
                                                                <img src="{{ $item->user->dataDiri ? asset('uploads/' . $item->user->dataDiri->foto_user) : asset('/images/default-images.svg') }}"
                                                                    alt="img">
                                                            </span>
                                                        </div>
                                                        <div class="flex-fill">
                                                            <h6 class="mb-1 fw-bold">{{ $item->user->name }}</h6>
                                                            <p class="mb-1 fs-12 text-muted text-uppercase">
                                                                {{ $item->user->dataDiri->kepegawaian->subJabatan->nama_sub_jabatan ?? 'Anggota' }}
                                                            </p>
                                                        </div>
                                                        @if ($config['is_manager'])
                                                            <div class="ms-2">
                                                                <form
                                                                    ...
                                                                    @csrf @method('DELETE')
                                                                    <button type="button"
                                                                        class="btn btn-icon btn-sm btn-outline-danger rounded-pill delete-anggota-project"
                                                                        data-id="{{ $item->id }}"
                    ...
                    <div class="modal-body">
                        @if (count($user) == 0)
                            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                                <strong>Perhatian! </strong> Harap menambahkan anggota task terlebih dahulu, di fitur
                                anggota
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="format-waktu_mulai"
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control" id="format-deadline_task"
                                        placeholder="Pilih Deadline" required>
                        ...
                        <div class="mb-3">
                            ...
                            <div id="preview-area" class="mt-2 text-center">
                                ...
                                <iframe id="previewPDF" src="" width="100%" height="300px"
                                    style="display:none;" class="rounded border"></iframe>
            ...
            if (calendarEl) {
                let calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    dateClick: function(info) {
                        ...
                        let filteredEvents = calendar.getEvents().filter(event => event.startStr
                            .startsWith(selectedDate));
                        let title = filteredEvents.length > 0 ? "Events on " + selectedDate :
                            "No events on " + selectedDate;
                        let body = filteredEvents.length > 0 ? filteredEvents.map(e =>
                            `<li><i class='ri-checkbox-blank-circle-fill text-primary me-2 fs-10'></i>${e.title}</li>`
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.updateTask', function(e) {
                ...
                if (fpTaskStart && tgl) {
                    fpTaskStart.setDate(tgl, true);
                    $("#tgl_task_modal").val(tgl);
                }
                if (fpTaskDeadline && ddl) {
                    fpTaskDeadline.setDate(ddl, true);
                    $("#deadline_task_modal").val(ddl);
                }
                ...
                if (upload) {
                    ...
                    if (['jpg', 'jpeg', 'png'].includes(ext)) $("#previewImage").attr("src", fileUrl)
                        .show();
                    else if (ext === 'pdf') $("#previewPDF").attr("src", fileUrl).show();
                    $("#detail_upload").html(
                        `<a href="${fileUrl}" target="_blank" class="text-primary">Lihat File Lama</a>`);
                } else {
